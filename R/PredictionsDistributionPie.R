...
PredictionsDistributionPie <- function(InputMatrix, pdf_output, pdfheight, 
    pdfwidth) {
  ...
  if(is.logical(pdf_output)) {
    if(pdf_output==TRUE) {
      if((is.numeric(pdfheight))&&(is.numeric(pdfwidth))) {
        pdf(file="MM2S_OverallPredictions_PieDist.pdf", height=pdfheight, 
          width=pdfwidth)
        pie(c(table(InputMatrix$MM2S_Subtype[,2])), main="Overall Predictions",
          col=colorscheme)
        dev.off()
      } else {
        stop("PDF dimensions must be numeric")
      }
    }
  } else {
    stop("TRUE or FALSE needed for PDF output")
  }
  pie(c(table(InputMatrix$MM2S_Subtype[,2])), main="Overall Predictions",
    col=colorscheme)
}
